...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_len=5000):
        ...
        deno = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        residual = x
        sa_out, _ = self.self_attn(x, x, x, mask=mask)
        x = self.norm1(residual + self.dropout(sa_out))
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask=None, tgt_mask=None):
        ...
        # Cross attention
        residual = x
        ca_out, _ = self.cross_attn(x, encoder_output, encoder_output, mask=src_mask)
        x = self.norm2(residual + self.dropout(ca_out))
...
class Seq2SeqTransformer(nn.Module):
    ...
    def forward(self, src, tgt, src_mask=None, tgt_mask=None):
        ...
        dec_out = self.decode(tgt, x, src_mask, tgt_mask)
        logits = self.output_projection(dec_out)
        return logits

    def generate(self, src, max_len=20, start_token=0):
        """
        Autoregressively generate sequence using greedy decoding.
        """
        self.eval()
        device = src.device
